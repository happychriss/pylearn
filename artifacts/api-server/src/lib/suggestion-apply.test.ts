...
test("findMatch: unique exact match", () => {
  ...
  assert.deepEqual(m, { index: 4, length: 3 });
});

test("findMatch: ambiguous exact match returns null", () => {
  assert.equal(findMatch("x = 1\nx = 1", "x = 1"), null);
});
...
test("findMatch: whitespace-normalised fallback", () => {
  ...
  assert.ok(m && content.slice(m.index, m.index + m.length).includes("print('hi')"));
});
...
test("applyChanges: ambiguous non-indent change errors", () => {
